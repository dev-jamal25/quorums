...
3. **Publish is idempotent, keyed by `contentItemId`,** enforced by a pre-publish guard (DL-022/039). A retried Hangfire segment must never double-post.
...
- `references/meta-integration.md` — the two-step `IMetaIntegration` contract, the `PublishResult` failure taxonomy, the idempotency guard, retry config, and mock requirements (DL-038/039).
