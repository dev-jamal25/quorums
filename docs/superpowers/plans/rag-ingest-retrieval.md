...
**Slice: RAG ingest + dense retrieval (slice 2) — brand-isolated thin loop (2026-06-14, branch `feat/rag-ingest-retrieval`, commits `1461dd4`…`119d63c`)**

- **Schema:** `KnowledgeDoc`/`KnowledgeChunk` extended; one migration `20260614194356_KnowledgeVectorSchema` adds `embedding vector(768)`, a generated `search_vector tsvector` (`GENERATED ALWAYS … STORED`) + GIN index, an HNSW `vector_cosine_ops` index, `DocType`/`Facet` enums (text), and `jsonb` metadata + doc `source`. RLS already covered both tables (InitialCreate) — no RLS SQL added. EF emits the `CREATE EXTENSION vector` via `HasPostgresExtension`; the generated column + HNSW/GIN go in via raw `migrationBuilder.Sql`.
- **Spike result (Task 1, deleted):** confirmed `Pgvector` **0.3.2** + `Pgvector.EntityFrameworkCore` **0.3.0** bind on EF Core **9.0.2** / Npgsql **9.0.2**; `UseNpgsql(cs, o => o.UseVector())` with a **bare connection string suffices** (no `NpgsqlDataSource`); `.OrderBy(c => c.Embedding!.CosineDistance(q))` (extension on `Pgvector.Vector`) translates to `<=>`; the generated tsvector is left **unmapped** and read via `SqlQueryRaw`. **Model-wide consequence:** every `AppDbContext` construction (design-time factory + all test fixtures) needed `UseVector()` once `Vector` is mapped.
- **Embedding:** `IEmbeddingProvider` (two methods — the prefix is the contract) with `NomicEmbeddingProvider` (typed `HttpClient`→`tei-embed`, prefix split, returned-dim == configured-dim guard) and `DeterministicEmbeddingProvider` (offline CI mock, hashed token-frequency vectors → shared vocabulary is nearest). Gated by `Embeddings:Mode`.
- **Ingest:** type-dispatched chunker (whole-unit / section-aware window, with the `market_intel` competitor sub-dispatch), `KnowledgeIngestService` true upsert keyed by `DeterministicGuid(docId, index)` — load tracked → mutate-or-add → remove orphans on shrink → one `SaveChanges` (avoids EF's delete+add-same-PK conflict), Hangfire `IngestKnowledgeDocJob`, `KnowledgeController` POST/PUT (enqueue) + DELETE (synchronous purge — no FK cascade).
- **Retrieval:** `PgVectorRetrieval` dense cosine top-k behind the `RetrievalOptions` stage seam (sparse/rerank/query-transform present-but-off). Brand isolation = RLS via the bound `BrandScope`, **never a manual `WHERE brand_id`**; `docType` = explicit filter; empty recall → `Grounded=false`, provider failure → `ToolError`.
- **Seed:** two-brand coffee-roaster corpus (`CoffeeRoasterCorpus` — identical content per brand so the leakage proof is separated by RLS alone), repeatable `KnowledgeSeeder` (deterministic doc ids), with real non-null `engagement_rate`/`ctr`/`audience_segment` on historical posts (slice-3 rerank demo); dev-only seed endpoint.
- **Proofs (all green):** isolation (zero B leakage on the dense arm, both directions, verified via a superuser context, vacuity-guarded), prefix correctness (real provider over a recording handler), ingest idempotency + orphan-on-shrink, empty-degrade, dense relevance (mock full-path; opt-in live-TEI test skipped in CI).
- **Gate evidence:** `dotnet build -warnaserror` → **0 warnings / 0 errors**; `dotnet format --verify-no-changes` → clean; `dotnet test Backend.sln` → **69 passed / 0 failed / 1 skipped** (UnitTests 25, IntegrationTests 44; the skip is the opt-in live-TEI test) — stable across two consecutive runs after switching the RAG tests to a shared `ICollectionFixture` (fixed Testcontainer contention that flaked `StorageTests`); `Category=Isolation` → green; **empty-volume proof** (`docker compose down -v` → `ef database update`) applied all three migrations clean and `psql` confirmed `embedding vector(768)`, `search_vector` generated, HNSW + GIN indexes, and RLS enabled+forced on both knowledge tables; `gitleaks` → pre-commit hook **Passed** on every commit (standalone CLI not installed in this environment).
