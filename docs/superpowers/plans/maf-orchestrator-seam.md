...
### MAF API cheat-sheet (CONFIRMED by the Task 1 spike, 2026-06-14)
- **NuGet package:** `Microsoft.Agents.AI.Workflows` **1.10.0** — it *is* its own package (the workflow/graph engine), not a namespace. "MAF 1.0" = the 1.x GA line; 1.10.0 is the latest stable. Brings `Microsoft.Agents.AI.Abstractions` 1.10.0 transitively. Pinned in `Directory.Packages.props` under CPM. Namespace: `Microsoft.Agents.AI.Workflows`.
- **Executor base + handler:** `sealed class X : Executor<TIn, TOut>` with primary ctor passing the id → `Executor<TIn,TOut>(id)`, and `public override ValueTask<TOut> HandleAsync(TIn message, IWorkflowContext context, CancellationToken cancellationToken = default)`. **No `[MessageHandler]` attribute and no `Microsoft.Agents.AI.Workflows.Generators` package** — the generic base is the supported path. (`ReflectingExecutor<T>` is legacy; not used.)
- **Edges / builder:** `new WorkflowBuilder(startExecutor)` (implicit `Executor`→`ExecutorBinding` conversion lets you pass executors directly); `.AddEdge(from, to)`; `.AddFanOutEdge(from, IEnumerable<ExecutorBinding>)`; **`.AddFanInBarrierEdge(IEnumerable<ExecutorBinding> sources, ExecutorBinding target)`** (NOT `AddFanInEdge`); `.WithOutputFrom(params ExecutorBinding[])` to designate the terminal output node; `.Build()` → `Workflow`.
- **Fan-in is a barrier that *streams each source message individually*** — it does NOT deliver a list. To merge, the join node is **`AggregatingExecutor<TInput, TAggregate>(string id, Func<TAggregate, TInput, TAggregate> aggregator)`**: the aggregator is an incremental fold, `agg` is **null on the first message**, then carries the running merge. Spike proved one fan-out + barrier + aggregator yields exactly **one** merged `WorkflowOutputEvent` that saw both branches.
- **Run + capture-output:** `Run run = await InProcessExecution.RunAsync(workflow, input);` then scan `run.NewEvents` for `WorkflowOutputEvent oe where oe.ExecutorId == terminalId && oe.Is<RunState>()` and read `oe.As<RunState>()`. Terminal nodes surface their output via `WithOutputFrom` + the default `ExecutorOptions.AutoYieldOutputHandlerResultObject` (handler return value is auto-yielded); `context.YieldOutputAsync(result, ct)` is the explicit equivalent.

### Confirmed reconciliations — these OVERRIDE the assumed samples in Tasks 2–5
- Every agent node: `sealed class FooExecutor(deps...) : Executor<RunState, RunState>("node-id")` with `public override ValueTask<RunState> HandleAsync(RunState state, IWorkflowContext ctx, CancellationToken ct = default)`. Keep the unit-testable `RunAsync(RunState, CancellationToken)` and have `HandleAsync` delegate to it (`=> new(RunAsync(state, ct))` for sync-bodied nodes; `await`/`return` for the I/O nodes).
- **AssemblyExecutor is an `AggregatingExecutor<RunState, RunState>`**, not a plain executor taking a list. Its aggregator folds the (≤2) branch RunStates by disjoint ownership: first message seeds the aggregate; the second merges `Caption`↔`Media`, unions `Errors` by value and `Trace.Spans` by `SpanId`, then sets `Draft` + `Phase = AwaitingApproval`. Build the merge as a pure static `Merge(RunState agg, RunState msg)` helper so it is unit-testable and order-independent (handle either arrival order). Designate it via `.WithOutputFrom(assembly)`.
- The two stub nodes (Ads/Analytics) are `Executor<RunState, RunState>` returning the not-implemented `ToolError` marker; constructed but **not added to any edge** (off the active path; `Build(validateOrphans: false)` so an unreachable bound node does not fail validation — or simply do not `Bind` them).
- `GenerationWorkflowFactory`: `new WorkflowBuilder(entry).AddEdge(entry, strategy).AddEdge(strategy, creative).AddFanOutEdge(creative, [copy, media]).AddFanInBarrierEdge([copy, media], assembly).WithOutputFrom(assembly).Build()`.
- `PublishWorkflowFactory`: `new WorkflowBuilder(publishing).WithOutputFrom(publishing).Build()`.
- `MafWorkflowRunner.RunToOutputAsync(workflow, state, terminalId, ct)` uses the `InProcessExecution.RunAsync` + `run.NewEvents` scan above; throws `InvalidOperationException` if no matching `WorkflowOutputEvent` was produced.
