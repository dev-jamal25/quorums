...
## Confirmed contracts (FILL IN from the Task 1 spike, then delete the spike)

> Task 1 writes a throwaway spike, confirms each item below against the **real** container/packages, records the confirmed shape here, then deletes the spike. Tasks 2+ are written against the **assumed** shapes; reconcile each before reuse. If a name/signature moved, **update this plan's samples in-place and continue.**

- **tei-rerank `/rerank` contract (assumed):** `POST /rerank` body `{"query": "<original query>", "texts": ["<doc0>", "<doc1>", …], "raw_scores": false}` → `200` with `[{"index": <int>, "score": <double>}, …]` **sorted by score descending**; `index` is the position in the request `texts[]`. Confirm: exact keys (`query`+`texts`), whether scores are sigmoid `[0,1]` (`raw_scores:false`) or logits, and the ordering. Record the confirmed request/response JSON.
- **FTS query shape (assumed):** project **`id` + the `ts_rank_cd` score** (NOT bare rows), so the FTS rank survives into the candidate (item 4): `SELECT id, ts_rank_cd(search_vector, websearch_to_tsquery('english', {0})) AS rank FROM knowledge_chunks WHERE search_vector @@ websearch_to_tsquery('english', {0}) [AND doc_type = {1}] ORDER BY rank DESC LIMIT {2}` via `_db.Database.SqlQueryRaw<FtsHit>(...)` (RLS-scoped on the bound connection), then a scoped `_db.KnowledgeChunks.Where(c => ids.Contains(c.Id))` re-read to materialize entities, re-attaching `rank` as `SparseScore`. Confirm: that `SqlQueryRaw<FtsHit>` (a `record FtsHit(Guid Id, double Rank)`) binds cleanly, parameter binding for the docType filter, and that `ts_rank_cd` + `websearch_to_tsquery('english', …)` run against the GIN-indexed column. (`FromSqlRaw<KnowledgeChunk>` is rejected: it cannot surface the computed `rank` — JC-1.)
- **Migration-free proof (assumed):** after all slice-3 C# changes, `dotnet ef migrations add ProbeNoOp -p backend/src/Infrastructure -s backend/src/Api` yields an **empty** `Up()`/`Down()`. Confirm, then `dotnet ef migrations remove` the probe. (JC-1.)
- **Sparse-arm RLS (assumed):** the raw-SQL FTS read (`SqlQueryRaw`) on the `BrandScope`-bound `AppDbContext` connection runs inside the transaction-scoped `set_config('app.current_brand', …, true)`, so it is RLS-scoped (the carve-out in `.claude/rules/infrastructure.md` permits exactly this read-only FTS use). **Demonstrate** (bind A, FTS-query a term present in both brands, assert zero B rows via a superuser cross-check) — don't assume.
- **Union/dedup shape (assumed):** dense top-N (existing LINQ) ∪ sparse top-N (raw-SQL projection) deduped by `chunk.Id` into one recall set (unranked). Record the confirmed candidate carrier (must carry `Id, DocId, Content, DocType, Facet, Metadata`, plus **`DenseScore` and `SparseScore`** for the rerank-off ordering — item 4).
- **`IChatClient` (Anthropic-backed) (assumed):** pin `Microsoft.Extensions.AI` (the `IChatClient` abstraction) + an Anthropic-backed provider that builds an `IChatClient`. The transformer injects `IChatClient` and calls `await chat.GetResponseAsync(prompt, new ChatOptions { ModelId = <config string>, MaxOutputTokens = 256 }, ct)`, then reads `response.Text`. Confirm: the exact provider package + `IChatClient` builder registration; that `ChatOptions.ModelId` takes the bare config string (`"claude-haiku-4-5"`); and that it binds on .NET 10 (`dotnet build -warnaserror`). Compile-only — CI never calls Claude (the mock transformer covers CI/ablation). This is the single Claude-call abstraction the generation slice will reuse (item 6).
...
- [ ] **Step 1: tei-rerank `/rerank` contract — against the live container.** Bring up `tei-rerank` (`docker compose up tei-rerank`; allow `start_period: 120s` for first-run weights) and probe:
