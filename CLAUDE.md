...
- **Pin the `tei-embed` model revision:** the command MUST carry `--revision bf8779d6d6e2b2b40132bc9266cc056a342aa4ed`. Unpinned `nomic-ai/nomic-embed-text-v1.5` `config.json` churns on `main` (a "standardize for Transformers" change adds a duplicate `max_position_embeddings`), and TEI 1.6's strict parser rejects duplicate keys → `Failed to parse config.json` → `tei-embed` exits(1), which aborts the whole `up` (api/worker/frontend never leave `Created` behind the `service_healthy` gate). That SHA is the verified-clean NomicBert config; never drop the pin. Same model + image, so DL-016/DL-024 hold.
